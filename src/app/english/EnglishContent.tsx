...
function getSourceTitle(source: string, titles: SourceTitles): string {
  if (source === "고전") return titles.classic ? `고전 — '${titles.classic}'` : "고전";
  if (source === "명화") return titles.art ? `예술 — '${titles.art}'` : "예술";
  if (source === "세계문화") return titles.world ? `세계 — '${titles.world}'` : "세계";
  if (source === "왜왜왜") return titles.why ? `과학 — '${titles.why}'` : "과학";
  return source;
}

export default function EnglishContent({ item, dayNumber, sourceTitles }: { item: EnglishItem | null; dayNumber: number; sourceTitles: SourceTitles }) {
  ...
  if (!item) return (
    <div className="theme-english min-h-screen px-4 py-8 max-w-lg mx-auto" style={{ background: "var(--background)" }}>
      <DayNavigator title="오늘의 영어" emoji="📝" topicKey="english" dayNumber={dayNumber} />
          ...
          <div className="w-full bg-white rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-semibold text-[var(--accent)] text-sm">
                {getSourceTitle(s.source, sourceTitles)}
              </span>
            </div>
